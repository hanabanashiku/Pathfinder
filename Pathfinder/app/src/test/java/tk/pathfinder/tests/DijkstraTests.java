...
public class DijkstraTests {
    ...
    public void SetUp(){
        ...
        nodes.add(new Room(1, new Point(1, 1, 4), "1", null, false)); // 0, ROOM 1
        nodes.add(new Intersection(2, new Point(1, 1, 3))); // 1
        edges.add(new Edge(
                new Room(3, new Point(1, 1, 5), "1", null, false),
                new Intersection(4, new Point(1, 1, 3))
        ));

        nodes.add(new Intersection(4, new Point(3, 1, 3))); // 2
        edges.add(new Edge(nodes.get(1), nodes.get(2)));

        nodes.add(new Room(5, new Point(3, 1, 2), "2", null, false)); // 3, ROOM 2
        edges.add(new Edge(nodes.get(2), nodes.get(3)));

        nodes.add(new Intersection(6, new Point(7, 1, 3))); // 4
        edges.add(new Edge(nodes.get(2), nodes.get(4)));

        nodes.add(new Room(7, new Point(7, 1, 2), "3", null, false)); // 5, ROOM 3
        edges.add(new Edge(nodes.get(4), nodes.get(5)));

        nodes.add(new Room(8, new Point(7, 1, 4), "4", null, false)); // 6, ROOM 4
        edges.add(new Edge(nodes.get(4), nodes.get(6)));

        nodes.add(new Intersection(9, new Point(10, 1, 3))); // 7
        ...
        // 8, Stairs
        nodes.add(new FloorConnector(10, new Point(10, 1, 5), "Stairs", FloorConnector.FloorConnectorTypes.STAIRCASE, new int[] {1, 2}, false, false));
        edges.add(new Edge(nodes.get(7), nodes.get(8)));

        ////////// FLOOR 2 //////////
        nodes.add(new Intersection(11, new Point(10, 2, 3))); // 9
        edges.add(new Edge(nodes.get(8), nodes.get(9)));

        nodes.add(new Room(12, new Point(2, 2, 3), "5", null, false)); // 10, ROOM 5
        edges.add(new Edge(nodes.get(9), nodes.get(10)));

        map = new Map(-1, "St. Davinceberg", edges.toArray(new Edge[]{}), null);
    ...
    public void NoPathTest() throws NoValidPathException {
        Room r = new Room(0, new Point(0, 0, 0), "50", "The Room That Should Not Be", true);
